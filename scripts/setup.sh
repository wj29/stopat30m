...
Options:
  ...
  --data-source SRC   数据源: baostock(默认) / qlib / akshare / tushare
...
if [[ -d "$QLIB_DATA_DIR" ]] && [[ "$(ls -A "$QLIB_DATA_DIR" 2>/dev/null)" ]]; then
    ...
    echo "   如需增量更新，运行: $PY main.py download --source $DATA_SOURCE --append"
else
...
echo "  增量更新:   python main.py download --source baostock --append"
