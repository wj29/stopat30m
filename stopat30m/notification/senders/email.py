...
from stopat30m.notification.formatters import markdown_to_html_document
...
def _parse_stock_email_groups(raw: Any) -> list[tuple[list[str], list[str]]]:
    """Normalize YAML list of dicts or pairs into (stocks, emails) tuples."""
    if not raw:
        return []
    out: list[tuple[list[str], list[str]]] = []
    for item in raw:
        if isinstance(item, dict):
            stocks = item.get("stocks") or item.get("stock_codes") or []
            emails = item.get("emails") or item.get("receivers") or []
            out.append((list(stocks), list(emails)))
...
class EmailSender:
    def __init__(self) -> None:
        sender = get("notification.email_sender") or ""
        self._email_config = {
            "sender": sender,
            "sender_name": get("notification.email_sender_name", default="stopat30m分析助手"),
            "password": get("notification.email_password") or "",
            "receivers": get("notification.email_receivers") or ([sender] if sender else []),
        }
        self._stock_email_groups = _parse_stock_email_groups(get("notification.stock_email_groups"))
    ...
    def get_receivers_for_stocks(self, stock_codes: List[str]) -> List[str]:
        ...
        normalized_codes = [normalize_instrument(c) for c in stock_codes]
        seen: set[str] = set()
        result: List[str] = []
        for stocks, emails in self._stock_email_groups:
            norm_stocks = [normalize_instrument(s) for s in stocks]
            for code in normalized_codes:
                if code in norm_stocks:
                    for e in emails:
                        if e not in seen:
                            seen.add(e)
                            result.append(e)
