...
from stopat30m.notification.formatters import chunk_content_by_max_bytes, slice_at_max_bytes


class CustomWebhookSender:
    def __init__(self) -> None:
        urls = get("notification.custom_webhook_urls")
        if isinstance(urls, str):
            self._custom_webhook_urls = [urls] if urls.strip() else []
        elif isinstance(urls, list):
            self._custom_webhook_urls = urls
        else:
            self._custom_webhook_urls = []
        self._custom_webhook_bearer_token = get("notification.custom_webhook_bearer_token")
