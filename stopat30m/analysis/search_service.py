...
# Stdlib logger: ``tenacity.before_sleep_log`` requires ``logging.Logger`` protocol.
_retry_log = logging.getLogger(__name__)


class _PrintfStyleLogger:
    """Stdlib-style ``%`` formatting delegated to loguru (DSA ``logging`` compatibility)."""
    ...
    def _fmt(msg: Any, args: Tuple[Any, ...]) -> str:
        ...
        if not isinstance(msg, str):
            return str(msg) + " " + " ".join(str(a) for a in args)
        try:
            return msg % args
        except Exception:
            return str(msg) + " | " + repr(args)

    def debug(self, msg: Any, *args: Any, **kwargs: Any) -> None:
        _loguru_logger.opt(depth=1).debug(self._fmt(msg, args))

    def info(self, msg: Any, *args: Any, **kwargs: Any) -> None:
        _loguru_logger.opt(depth=1).info(self._fmt(msg, args))

    def warning(self, msg: Any, *args: Any, **kwargs: Any) -> None:
        _loguru_logger.opt(depth=1).warning(self._fmt(msg, args))

    def error(self, msg: Any, *args: Any, **kwargs: Any) -> None:
        _loguru_logger.opt(depth=1).error(self._fmt(msg, args))

    def critical(self, msg: Any, *args: Any, **kwargs: Any) -> None:
        _loguru_logger.opt(depth=1).critical(self._fmt(msg, args))


logger = _PrintfStyleLogger()

_NEWS_STRATEGY_WINDOWS = {"short": 3, "medium": 7, "long": 14}


def _normalize_news_strategy_profile(news_strategy_profile: Optional[str]) -> str:
    raw = (news_strategy_profile or "short").strip().lower()
    if raw in _NEWS_STRATEGY_WINDOWS:
        return raw
    return "short"


def _resolve_news_window_days(
    news_max_age_days: int = 3,
    news_strategy_profile: str = "short",
) -> int:
    return _NEWS_STRATEGY_WINDOWS.get(news_strategy_profile, news_max_age_days)
...
def fetch_url_content(url: str, timeout: int = 5) -> str:
    ...
    try:
        ...
        logger.debug("newspaper3k 未安装，fetch_url_content 不可用: pip install newspaper3k")
        return ""
...
class SearchService:
    ...
    def _is_us_stock(cls, stock_code: str) -> bool:
        ...
        return bool(cls._US_STOCK_RE.match(code))
    ...
    def _effective_news_window_days(self) -> int:
        ...
        return _resolve_news_window_days(
            news_max_age_days=self.news_max_age_days,
            news_strategy_profile=self.news_strategy_profile,
        )
...
def get_search_service() -> SearchService:
    ...
    if _search_service is None:
        with _search_service_lock:
            if _search_service is None:
                from stopat30m.config import get as _cfg_get

                def _csv_keys(key: str) -> List[str]:
                    raw = _cfg_get("search", key, "") or ""
                    return [x.strip() for x in str(raw).split(",") if x.strip()]

                def _keys(primary: str, *legacy: str) -> List[str]:
                    keys = _csv_keys(primary)
                    if keys:
                        return keys
                    for lk in legacy:
                        s = str(_cfg_get("search", lk, "") or "").strip()
                        if s:
                            return [s]
                    return []

                _search_service = SearchService(
                    bocha_keys=_keys("bocha_api_keys", "bocha_api_key"),
                    tavily_keys=_keys("tavily_api_keys", "tavily_api_key"),
                    anspire_keys=_csv_keys("anspire_api_keys"),
                    brave_keys=_csv_keys("brave_api_keys"),
                    serpapi_keys=_csv_keys("serpapi_keys"),
                    minimax_keys=_csv_keys("minimax_api_keys"),
                    searxng_base_urls=_csv_keys("searxng_base_urls"),
                    searxng_public_instances_enabled=bool(
                        _cfg_get("search", "searxng_public_instances_enabled", True)
                    ),
                    news_max_age_days=int(_cfg_get("search", "news_max_age_days", 3)),
                    news_strategy_profile=str(
                        _cfg_get("search", "news_strategy_profile", "short") or "short"
                    ),
                )
