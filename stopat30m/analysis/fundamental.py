...
def _normalize_code(raw: Any) -> str:
    return bare_code(_safe_str(raw))
...
class AkshareFundamentalAdapter:
    ...
    def get_fundamental_bundle(self, stock_code: str) -> Dict[str, Any]:
        ...
        # Valuation (PE/PB/MV) from East Money individual info
        val_df, val_source, val_errors = self._call_df_candidates([
            ("stock_individual_info_em", {"symbol": stock_code}),
        ])
        ...
        if val_df is not None:
            ...
            for _, row in val_df.iterrows():
                ...
                val = row.iloc[1]
                if "市盈率" in key:
                    val_map["pe_ratio"] = _safe_float(val)
                elif "市净率" in key:
                    val_map["pb_ratio"] = _safe_float(val)
                elif "总市值" in key:
                    val_map["total_mv"] = _safe_float(val)
                elif "流通市值" in key:
                    val_map["circ_mv"] = _safe_float(val)
                elif "换手率" in key:
                    val_map["turnover_rate"] = _safe_float(val)
            if any(v is not None for v in val_map.values()):
                result["valuation"] = val_map
                result["source_chain"].append(f"valuation:{val_source}")
        ...
        has_content = bool(
            result["growth"] or result["earnings"] or result["institution"] or result.get("valuation")
        )
...
def fetch_fundamental_context(code: str) -> dict | None:
    """Build fundamental bundle via ``AkshareFundamentalAdapter.get_fundamental_bundle``."""
    sym = bare_code(code)
    adapter = AkshareFundamentalAdapter()
    bundle = adapter.get_fundamental_bundle(sym)
    if bundle.get("valuation"):
        bundle["realtime"] = bundle["valuation"]
    if (
        bundle.get("status") == "not_supported"
        and not bundle.get("growth")
        and not bundle.get("earnings")
        and not bundle.get("institution")
        and not bundle.get("valuation")
    ):
        return None
    return bundle
